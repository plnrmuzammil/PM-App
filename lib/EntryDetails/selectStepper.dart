...
class _StepperFormState extends State<StepperForm> {
  ...
  Future<void> submitForm() async {
    ...
    try {
      ...
      final docRef = await db.add({
        ...
        "scheme": selectedScheme, // corrected
        "schemeName": selectedScheme, // corrected
        "typeId": selectedTypeId, // added for clarity
        "typeName": selectedType, // added for clarity
        ...
        "name": username,
        "time": DateTime.now(),
        "propertyImagesURL": uploadedImagesUrls,
  ...
  Future<String?> uploadFile(File file) async {
    try {
      ...
      final storageRef = FirebaseStorage.instance.ref().child("entries_uploads/$fileName");
      ...
      if (snapshot.hasData) {
        ...
        for (var i in snapshot.data.docs) {
          ...
          if (data['provinceID'] == selectedProvinceId) {
            districts.add(data['name']);
            districtId.add(data['id']);
          }
        }
        return DropDown(
  ...
  Widget _buildSchemeStep() => DropDown(
    ...
    onChanged: (val) {
      setState(() {
        ...
        _currentStep = _currentStep.clamp(0, getSteps().length - 1);
      });
    },
  );
      ...
      if (snapshot.hasData) {
        ...
        for (var i in snapshot.data.docs) {
          ...
          if (data['provinceID'] == selectedProvinceId &&
              ...
              data['schemeID'] == selectedTypeId) {
            phases.add(data['name']);
            phaseIds.add(data['id']);
          }
        }
        return DropDown(
      ...
      if (snapshot.hasData) {
        ...
        for (var i in snapshot.data.docs) {
          ...
          if (data['provinceID'] == selectedProvinceId &&
              ...
              data['schemeID'] == selectedTypeId &&
              data['phaseID'] == selectedPhaseId &&
              data['blockID'] == selectedBlockId) {
            subBlocks.add(data['name']);
            subBlockIds.add(data['id']);
          }
        }
        return DropDown(
  ...
  Widget _buildPropertyTypeStep() => DropDown(
  ...
  Widget _buildSubTypeStep() => DropDown(
    hint: const Text("Select Property Subtype"),
    items: subTypes,
    onChanged: (val) => setState(() => selectedSubType = val),
  );

  Widget _buildPurposeStep() => DropDown(
    hint: const Text("Select Purpose"),
    items: ["Lease", "Rent", "Sale"],
  ...
  Widget _buildPlotDetailsStep() => Column(
    children: [
      ...
      DropDown(
        ...
        items: ["Marla", "Squareft"],
        onChanged: (val) => setState(() => selectedUnit = val!),
      ),
      TextFormField(
        controller: demandController,
        decoration: const InputDecoration(labelText: "Demand", hintText: "Value of Demand"),
        keyboardType: TextInputType.number,
      ),
      TextFormField(
        controller: detailsController,
        decoration: const InputDecoration(
            labelText: "All Details: Number of Rooms, Washrooms, Lounge etc ",
            hintText: "All Details like: Numbers of Rooms, Kitchen, Washrooms, Lounge, etc and other condition of Property"),
        maxLines: 3,
      ),
      TextFormField(
        controller: plotController,
        decoration: const InputDecoration(labelText: "Plot / Flat / Khasra"),
      ),
      ...
      TextFormField(
        ...
        decoration: const InputDecoration(labelText: "Address", hintText: "Please Enter property location like street number etc"),
        maxLines: 2,
      ),
      MaterialButton(
        color: Colors.green,
